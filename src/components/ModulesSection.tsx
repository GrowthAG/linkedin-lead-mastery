...
import { BookOpen, Layout, Edit, TrendingUp, MessageSquare, BarChart, Zap, Gift, FileText, Monitor, CheckCircle, Calendar, Users, Target, Award, Clipboard } from 'lucide-react';
...
const modules = [{
  ...
  value: "R$297",
  icon: BookOpen,
  highlights: ["Identificação precisa do seu nicho de mercado", "Validação do potencial de monetização", "Posicionamento estratégico no LinkedIn"]
}, {
  number: 2,
  ...
  value: "R$297",
  icon: Layout,
  highlights: ["Criação da sua marca pessoal profissional", "Definição do seu público-alvo ideal", "Estratégias de diferenciação no mercado"]
}, {
  number: 3,
  ...
  value: "R$297",
  icon: Edit,
  highlights: ["Implementação do modelo Hub and Spoke", "Integração de diferentes formatos de conteúdo", "Maximização do alcance orgânico"]
}, {
  number: 4,
  ...
  value: "R$397",
  icon: TrendingUp,
  highlights: ["Processo estruturado de geração de ideias", "Calendário editorial otimizado", "Técnicas de brainstorming efetivo"]
}, {
  number: 5,
  ...
  value: "R$197",
  icon: MessageSquare,
  highlights: ["Framework de escrita profissional", "Técnicas de copywriting para LinkedIn", "Otimização de tempo na produção de conteúdo"]
}, {
  number: 6,
  ...
  value: "R$397",
  icon: BarChart,
  highlights: ["Otimização completa do perfil para máxima conversão de visitantes", "Sequência de elementos persuasivos que transformam visualizações em mensagens", "Sistema de follow-up que converte conexões em reuniões comerciais"]
}, {
  number: 7,
  ...
  value: "R$297",
  icon: Zap,
  highlights: ["Como criar conteúdo viral em setores tradicionalmente conservadores", "Adaptações específicas para áreas técnicas, jurídicas e financeiras", "Estratégias avançadas de monetização para serviços de alto valor"]
}];
const bonuses = [{
  icon: FileText,
...
const ModulesSection = () => {
  ...
  return <section id="modules" className="py-16 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-8">
          {modules.map((module, index) => <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden card-hover">
              <div className="flex flex-col md:flex-row">
                ...
                <div className="p-6 md:w-2/3">
                  <ul className="space-y-3">
                    {module.highlights.map((highlight, idx) => <li key={idx} className="flex items-start">
        ...
        <div className="mt-20 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mb-12">
            {bonuses.map((bonus, index) => <Card key={index} className="bg-white border-2 border-[#1EAEDB] p-6 rounded-xl shadow-sm hover:shadow-md transition-all">
          ...
          <div className="bg-yellow-50 border-2 border-yellow-300 p-6 rounded-xl max-w-3xl mx-auto mb-10">
            ...
            <p className="text-lg font-medium mt-2">
              Mas hoje pode acessar por apenas <span className="text-linkedin-primary font-bold">R$297 à vista</span> ou 12x de R$29,90
            </p>
          </div>
          ...
          <Button className="cta-button text-lg bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white px-6 py-3 rounded-md font-bold shadow-lg transform transition-all duration-300 hover:scale-105 hover:shadow-xl" onClick={scrollToPrice}>
